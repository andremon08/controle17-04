...
function renderizarVereador(gab, verbaMes, servidores) {
    ...
    if (estrutura.length === 0) {
        ...
        // Cria cópia consumível dos cargos ativos para tracking de repetições
        const cargosAtivos = servidores
            .filter(s => !s.exoneradoNoMes)
            .map(s => extrairCC(s['Cargo'] || ''));

        // Para exonerados no mês, mantém como slot "exonerado"
        const cargosExonerados = servidores
            .filter(s => s.exoneradoNoMes)
            .map(s => extrairCC(s['Cargo'] || ''));

        const estruturaTrackeada = estrutura.map(cc => {
            const idxAtivo = cargosAtivos.indexOf(cc);
            if (idxAtivo !== -1) {
                cargosAtivos.splice(idxAtivo, 1);
                return { cc, estado: 'ocupado' };
            }
            const idxExo = cargosExonerados.indexOf(cc);
            if (idxExo !== -1) {
                cargosExonerados.splice(idxExo, 1);
                return { cc, estado: 'exonerado' };
            }
            return { cc, estado: 'vago' };
        });

        const vagasLivresEstrutura = estruturaTrackeada.filter(s => s.estado === 'vago').length;
        resumoEl.textContent = vagasLivresEstrutura > 0
            ? `${vagasLivresEstrutura} vaga(s) livre(s) de ${estrutura.length}`
            : `${estrutura.length} de ${estrutura.length} preenchidos`;

        grade.innerHTML = '';
        estruturaTrackeada.forEach(slot => {
            grade.innerHTML += renderBoneco(slot.cc, slot.estado);
        });
    }
...
// Gera o HTML de um boneco SVG com cargo e status
function renderBoneco(cc, estado) {
    const cls = estado; // 'ocupado', 'vago', 'exonerado'
    const statusTxt = estado === 'ocupado' ? 'Ocupado' : estado === 'exonerado' ? 'Exonerado' : 'Vago';
    return `
        <div class="boneco-slot ${cls}">
            <div class="boneco-figura">
                <div class="boneco-cabeca"></div>
                <div style="position:relative;display:flex;flex-direction:column;align-items:center">
                    <div class="boneco-corpo"></div>
                    <div class="boneco-bracos" style="position:absolute;top:4px"></div>
                    <div class="boneco-pernas">
                        <div class="boneco-perna boneco-perna-esq"></div>
                        <div class="boneco-perna boneco-perna-dir"></div>
                    </div>
                </div>
            </div>
            <span class="boneco-cc">${cc}</span>
            <span class="boneco-status">${statusTxt}</span>
        </div>`;
}
