...
function renderizarEstruturaCCs(estrutura, cargosAtivosCC) {
    // Agrupa slots por CC mantendo a ordem original e tracking de ocupação
    // cargosAtivosCC: array de CC codes dos servidores ativos (exonerados no mês = vago)
    const consumivel = [...cargosAtivosCC];

    // Monta lista de slots com estado
    const slots = estrutura.map(cc => {
        const idx = consumivel.indexOf(cc);
        if (idx !== -1) {
            consumivel.splice(idx, 1);
            return { cc, estado: 'ocupado' };
        }
        return { cc, estado: 'vago' };
    });

    // Agrupa por CC preservando a ordem de primeiro aparecimento
    const ordem = [];
    const grupos = {};
    slots.forEach(s => {
    ...
    ordem.forEach(cc => {
        ...
        // Título do grupo: quantidade + CC
        const tituloTexto = n > 1 ? `${n}× ${cc}` : cc;
        // Subtítulo com contagem de ocupados/vagos
        const subTexto = ocupados === n
            ? `${n}/${n} ocupado${n > 1 ? 's' : ''}`
            : ocupados === 0
                ? `${n}/${n} vago${n > 1 ? 's' : ''}`
                : `${ocupados}/${n} ocupado${ocupados > 1 ? 's' : ''}`;

        // Distribui os cards em linhas para disposição triangular/quadrada
        // Ex: 1→[1], 2→[2], 3→[2,1], 4→[2,2], 5→[3,2], 6→[3,3], 7→[4,3], 8→[4,4], 9→[5,4]
        ...
        html += `
            <div class="grupo-cc">
                <span class="grupo-cc-titulo">${tituloTexto}</span>
                <span style="font-size:9px;color:var(--muted);margin-top:-3px;margin-bottom:2px;font-family:var(--mono)">${subTexto}</span>
                ${linhasHTML}
            </div>`;
    });
